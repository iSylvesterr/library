...
local function IsFinite(p4) -- Line: 45
    local v5;
...
local function IsFiniteVector3(p6) -- Line: 49
    local x = p6.x;
    local v7;
...
function u8.SetLaserPointerMode(p12, u13) -- Line: 95
...
function u8.GetLocalHumanoid(p14) -- Line: 101
...
function u8.HasBothHandControllers(p15) -- Line: 115
...
function u8.HasAnyHandControllers(p17) -- Line: 120
    -- upvalues: VRService (copy)
...
function u8.IsMobileVR(p18) -- Line: 125
    -- upvalues: UserInputService (copy)
    return UserInputService.TouchEnabled;
end;

function u8.HasGamepad(p19) -- Line: 129
    -- upvalues: UserInputService (copy)
    return UserInputService.GamepadEnabled;
end;

function u8.ShouldUseNavigationLaser(p20) -- Line: 133
...
function u8.StartFollowingPath(p21, p22) -- Line: 153
...
function u8.GoToPoint(p24, p25) -- Line: 170
    -- upvalues: BindableEvent (copy)
    currentPath = true;
    currentPoints = { p25 };
    currentPointIdx = 1;
...
function u8.StopFollowingPath(p28) -- Line: 186
    currentPath = nil;
    currentPoints = nil;
    currentPointIdx = 0;
    moving = false;
...
function u8.TryComputePath(p29, p30, p31) -- Line: 194
...
function u8.OnNavigationRequest(p34, p35, p36) -- Line: 221
    -- upvalues: u1 (ref)
...
function u8.OnJumpAction(p40, p41, p42, p43) -- Line: 267
...
function u8.BindJumpAction(u44, p45) -- Line: 273
    -- upvalues: ContextActionService (copy)
    if p45 then
        if not u44.isJumpBound then
            u44.isJumpBound = true;
            ContextActionService:BindActionAtPriority("VRJumpAction", function() -- Line: 277
                -- upvalues: u44 (copy)
...
function u8.ControlCharacterGamepad(p46, p47, p48, p49) -- Line: 289
...
function u8.OnHeartbeat(p50, p51) -- Line: 334
    ...
    if p50.moving and p50.currentPoints then
        ...
        if magnitude < 1 then
            ...
            if u3 then
                u2.FilterDescendantsInstances = { game:GetService("Players").LocalPlayer.Character, workspace.CurrentCamera };
                ...
                local v59 = { game:GetService("Players").LocalPlayer.Character, workspace.CurrentCamera };
...
function u8.OnUserCFrameEnabled(p67) -- Line: 438
...
function u8.Enable(u68, p69) -- Line: 448
    -- upvalues: VRService (copy), RunService (copy), ContextActionService (copy)
    u68.moveVector = Vector3.new(0, 0, 0);
    u68.isJumping = false;

    if p69 then
        u68.navigationRequestedConn = VRService.NavigationRequested:Connect(function(p70, p71) -- Line: 454
            -- upvalues: u68 (copy)
            u68:OnNavigationRequest(p70, p71);
        end);
        u68.heartbeatConn = RunService.Heartbeat:Connect(function(p72) -- Line: 458
            -- upvalues: u68 (copy)
            u68:OnHeartbeat(p72);
        end);
        ContextActionService:BindAction("MoveThumbstick", function(p73, p74, p75) -- Line: 462
            -- upvalues: u68 (copy)
        ...
        u68.userCFrameEnabledConn = VRService.UserCFrameEnabled:Connect(function() -- Line: 467
            -- upvalues: u68 (copy)
