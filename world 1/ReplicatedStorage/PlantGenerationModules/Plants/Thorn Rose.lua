...
return {
    ...
    InitPlant = function(u1, p2, p3) -- Line: 8, Name: InitPlant
        ...
        for i = 1, 40 do
            ...
            for _, v in v37 do
                ...
                local v44 = v.Position + Normal * 0.5;
                local Magnitude = (v44 - Position).Magnitude;

                if Magnitude >= 0.3 then
                    local v45, v46, v47, v48, v49, v50, v51, v52, v53, v54, v55;

                    if Magnitude > 3.5999999999999996 then
                        v44 = Position + (v.Position - Position).Unit * 3;
                        local v56 = v44 + Normal * 2;
                        local v57 = -Normal;
                        local v58;
                        ...
                        if v57.Magnitude < 0.01 then
                            ...
                            v58 = workspace:Raycast(v56, v57.Unit * 4, u8);
                        end;

                        if v58 then
                            v44 = v58.Position + v58.Normal * 0.5;
                            Normal = v58.Normal;
                        end;

                        Magnitude = (v44 - Position).Magnitude;

                        if Magnitude >= 0.3 then
                            v45 = 1 - math.abs(Normal.Y);
                            v46 = (v44.Y - Position.Y) * 15 + v45 * 20 + (v44.Y - Y) * 2 + math.max(0, 15 - Magnitude) * 0.3;
                            v47 = v44 - Position;

                            if v47.Magnitude > 0.01 then
                                v46 = v46 + v47.Unit:Dot(Unit) * 2;
                            end;
                            ...
                            if v40 < v51 then
                                v52 = Vector3.new(0, 1, 0) - (Vector3.new(0, 1, 0)):Dot(Normal) * Normal;
                                ...
                                v43 = v52.Unit;
                                v53 = Unit - Unit:Dot(Normal) * Normal;

                                if v53.Magnitude < 0.01 then
                                    v53 = Normal:Cross(Vector3.new(1, 0, 0));

                                    if v53.Magnitude < 0.01 then
                                        v53 = Normal:Cross(Vector3.new(0, 0, 1));
                                    end;
                                end;

                                v54 = v43 * 0.7 + v53.Unit * 0.3;

                                if v54.Magnitude > 0.01 then
                                    v55 = v54 - v54:Dot(Normal) * Normal;

                                    if v55.Magnitude < 0.01 then
                                        v55 = Normal:Cross(Vector3.new(1, 0, 0));

                                        if v55.Magnitude < 0.01 then
                                            v55 = Normal:Cross(Vector3.new(0, 0, 1));
                                        end;
                                    end;
            ...
            if v41 then
                ...
                for _, v in {
                    (Unit - Vector3.new(0, 1, 0)).Unit,
                    Vector3.new(-0, -1, -0),
                    (Unit * 0.5 - Vector3.new(0, 1, 0)).Unit,
                    (-Unit * 0.5 - Vector3.new(0, 1, 0)).Unit,
                    (CFrame.fromAxisAngle(Vector3.new(0, 1, 0), 1.5707963267948966) * CFrame.new(Unit)).Position.Unit - Vector3.new(0, 0.5, 0),
                    (CFrame.fromAxisAngle(Vector3.new(0, 1, 0), -1.5707963267948966) * CFrame.new(Unit)).Position.Unit - Vector3.new(0, 0.5, 0)
                } do
                    if v.Magnitude > 0.01 then
                        local v60 = Position + v9 * 0.3;
                        local Unit2 = v.Unit;
                        local v61;
                        ...
                        if Unit2.Magnitude < 0.01 then
                            ...
                            v61 = workspace:Raycast(v60, Unit2.Unit * 30, u8);
                        end;

                        if v61 then
                            local v62 = v61.Position + v61.Normal * 0.5;

                            if (v62 - Position).Magnitude >= 0.3 then
                                v42 = v61.Normal;
                                local Normal = v61.Normal;
                                local v63 = Unit - Unit:Dot(Normal) * Normal;

                                if v63.Magnitude < 0.01 then
                                    v63 = Normal:Cross(Vector3.new(1, 0, 0));

                                    if v63.Magnitude < 0.01 then
                                        v63 = Normal:Cross(Vector3.new(0, 0, 1));
                                    end;
                                end;
            ...
            local v64 = v41 - Position;

            if v64.Magnitude < 0.1 then
                v41 = Position + v59 * 3;
                v64 = v41 - Position;
            end;

            local v65 = Stud_Part:Clone();
            v65.Size = Vector3.new(1, 1, v64.Magnitude);
            v65.CFrame = CFrame.lookAt((Position + v41) / 2, v41, v9);
            v65.Anchored = true;
            v65.CanCollide = false;
            v65.Transparency = 1;
            v65.Color = v5;
            v65.Name = tostring(u22);
            v65.Parent = u1;
            table.insert(u7, v65);
            attachAllThorns(v65.CFrame, v65.Size);

            if u4:NextNumber() <= 0.1 then
                local v66 = v65.CFrame.Position + v65.CFrame.UpVector * (v65.Size.Y / 2 + 0.5);
                local Part = Instance.new("Part");
                Part.Size = Vector3.new(1, 1, 1);
                Part.Transparency = 1;
                Part.Anchored = true;
                Part.CanCollide = false;
                Part.CFrame = CFrame.new(v66);
    ...
    BeginPlantGrowth = function(u67) -- Line: 301, Name: BeginPlantGrowth
        ...
        local function updateGrowth() -- Line: 331
            ...
            local v72 = u67:GetAttribute("Age") or 0;

            for _, v in u68 do
                local v73 = math.min(v72 - v.partAge, 1);
                ...
                if v74 ~= v.lastProgress then
                    ...
                    if v73 > 0 then
                        local v75 = v.maxSize.Z * v73;
                        v.part.Size = Vector3.new(v.maxSize.X, v.maxSize.Y, v75);
                        v.part.CFrame = PrimaryPart.CFrame * v.centerOffset * CFrame.new(0, 0, (v.maxSize.Z - v75) / 2);
                        v.part.Transparency = v.part:GetAttribute("OG_Transparency") or 0;
                        v.part.CanCollide = true;

                        for _, v2 in v.decals do
                            v2.decal.Transparency = v2.originalTransparency + (1 - v2.originalTransparency) * (1 - v73);
                        end;
                    else
            ...
            if game.Players.LocalPlayer and (game:GetService("RunService"):IsClient() and (not u67:GetAttribute("playedSfx") and u67:GetAttribute("MaxAge") <= v72)) then
                u67:SetAttribute("playedSfx", true);
