...
local function MAKE_LOOKUP(p2) -- Line: 68
    local v3 = {};
...
local function Q(p8) -- Line: 92
    return string.format("%q", p8):gsub("\r", "\\r"):gsub("\n", "\\n");
end;

local function formatError(p9, p10) -- Line: 98
    return string.format("%s at line %d col %d", p9, p10[1], p10[2]);
end;

local function advanceNewline(p11, p12) -- Line: 107
    -- upvalues: u6 (copy)
    if not u6[p11] then
        return false;
    end;
...
local function getNewline(p13, p14) -- Line: 122
...
local function getWhitespace(p16, p17) -- Line: 137
...
local function stripWhitespace(p18, p19) -- Line: 158
...
local function stripInlineComments(p21, p22) -- Line: 178
...
local function stripBlockComments(p24, p25) -- Line: 205
...
local function codepointToutf8(p27) -- Line: 232
    if p27 <= 127 then
...
local function parseUnicodeImpl(p34, p35) -- Line: 257
...
local function getSurrogatePair(p38, p39) -- Line: 277
    return (p39 - 55296) * 1024 + (p38 - 56320) + 65536;
end;

local function parseUnicode(p40, p41) -- Line: 283
...
local function parseStringImpl(p49, p50, p51, p52) -- Line: 316
...
local function parseString(p60, p61) -- Line: 395
    -- upvalues: parseStringImpl (copy)
    local u62 = p60:sub(1, 1);
    local v64, v65 = parseStringImpl(p60:sub(2), function(p63) -- Line: 403, Name: stopCriterion
        -- upvalues: u62 (copy)
...
local function parseNumber(p66, p67) -- Line: 416
...
local function parseBoolean(p77, p78) -- Line: 501
    -- upvalues: formatError (copy)
...
local function stripComments(p79, p80) -- Line: 518
...
local function stripWhitespaceAndComments(p82, p83) -- Line: 536
...
local function testIdentifier(p86) -- Line: 610
...
local function stopIdentifier(p89) -- Line: 649
    -- upvalues: getWhitespace (copy)
    return p89:sub(1, 1) == ":" and true or getWhitespace(p89, { 0, 0 }) ~= nil;
end;

local function parseIdentifier(p90, p91) -- Line: 657
...
local u127 = {
    ...
    n = function(p97, p98, p99) -- Line: 486, Name: parseNull
        -- upvalues: formatError (copy)
    ...
    ["["] = function(p100, p101, p102) -- Line: 564, Name: parseArray
    ...
    ["{"] = function(p110, p111, p112) -- Line: 685, Name: parseObject
    ...
    [""] = function(p125, p126) -- Line: 752, Name: catchEOF
        -- upvalues: formatError (copy)
        error(formatError("unexpected eof", p126));
    end
};

u85 = function(p128, p129, p130) -- Line: 786
...
local v139 = {
    ...
    Decode = function(p134, p135) -- Line: 821, Name: Decode
        -- upvalues: stripWhitespaceAndComments (copy), u85 (ref), formatError (copy)
        local v136 = { 1, 1 };
