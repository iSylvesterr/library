...
local function fireBurstEmitters(p38) -- Line: 182
    for _, descendant in p38:GetDescendants() do
        if descendant:IsA("ParticleEmitter") then
            local v39 = descendant:GetAttribute("EmitDuration");

            if typeof(v39) ~= "number" or v39 <= 0 then
                local v40 = descendant:GetAttribute("EmitCount");
                local u41 = typeof(v40) ~= "number" and 1 or v40;
                local v42 = descendant:GetAttribute("EmitDelay");
                local v43 = typeof(v42) ~= "number" and 0 or v42;

                if v43 > 0 then
                    task.delay(v43, function() -- Line: 194
                        -- upvalues: descendant (copy), u41 (copy)
