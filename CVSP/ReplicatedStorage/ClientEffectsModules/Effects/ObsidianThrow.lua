...
local Obsidian = EffectAssets:WaitForChild("Obsidian");
local ObsidianHit = EffectAssets:WaitForChild("ObsidianHit");
...
return {
    Play = function(p1) -- Line: 21, Name: Play
        -- upvalues: Obsidian (copy), Visuals (copy), HeavyThrow (copy), RunService (copy), ObsidianHit (copy), HeavyRockHit (copy), Debris (copy)
        local Target = p1.Target;
        local TowerPosition = p1.TowerPosition;
        local TargetPosition = p1.TargetPosition;
        local u2 = Obsidian:Clone();
        ...
        if u2 then
            ...
            u7 = RunService.Heartbeat:Connect(function() -- Line: 47
                -- upvalues: u4 (copy), TowerPosition (copy), u6 (ref), TargetPosition (copy), u2 (copy), u7 (ref), ObsidianHit (ref), Target (copy), HeavyRockHit (ref), Visuals (ref), Debris (ref)
