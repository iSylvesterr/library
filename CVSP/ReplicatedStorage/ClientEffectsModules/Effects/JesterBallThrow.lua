...
local Ball = EffectAssets:WaitForChild("Ball");
local BallHit = EffectAssets:WaitForChild("BallHit");
local Boing = script:WaitForChild("Boing");
local JesterThrow = script:WaitForChild("JesterThrow");
local Visuals = workspace:WaitForChild("World"):WaitForChild("Visuals");

return {
    Play = function(p1) -- Line: 21, Name: Play
        -- upvalues: Ball (copy), Visuals (copy), JesterThrow (copy), RunService (copy), BallHit (copy), Boing (copy), Debris (copy)
        ...
        if u2 then
            ...
            u7 = RunService.Heartbeat:Connect(function() -- Line: 47
                -- upvalues: u4 (copy), TowerPosition (copy), u6 (ref), TargetPosition (copy), u2 (copy), u7 (ref), BallHit (ref), Target (copy), Boing (ref), Visuals (ref), Debris (ref)
