...
local GoldBar = EffectAssets:WaitForChild("GoldBar");
local GoldHit = EffectAssets:WaitForChild("GoldHit");
local DiamondHit = script:WaitForChild("DiamondHit");
...
return {
    Play = function(p1) -- Line: 20, Name: Play
        -- upvalues: GoldBar (copy), Visuals (copy), Throw (copy), RunService (copy), GoldHit (copy), DiamondHit (copy), Debris (copy)
        local Target = p1.Target;
        local TowerPosition = p1.TowerPosition;
        local TargetPosition = p1.TargetPosition;
        local u2 = GoldBar:Clone();
        ...
        if u2 then
            ...
            u7 = RunService.Heartbeat:Connect(function() -- Line: 43
                -- upvalues: u4 (copy), TowerPosition (copy), u6 (ref), TargetPosition (copy), u2 (copy), u7 (ref), GoldHit (ref), Target (copy), DiamondHit (ref), Visuals (ref), Debris (ref)
