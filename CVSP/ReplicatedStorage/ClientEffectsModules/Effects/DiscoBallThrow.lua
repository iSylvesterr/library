...
local DiscoBall = EffectAssets:WaitForChild("DiscoBall");
local DiscoBallHit = EffectAssets:WaitForChild("DiscoBallHit");
local ImpactGlass = script:WaitForChild("ImpactGlass");
local Throw = script:WaitForChild("Throw");
local Beat = script:WaitForChild("Beat");
local Visuals = workspace:WaitForChild("World"):WaitForChild("Visuals");

return {
    Play = function(p1) -- Line: 22, Name: Play
        -- upvalues: DiscoBall (copy), Visuals (copy), Throw (copy), RunService (copy), DiscoBallHit (copy), ImpactGlass (copy), Beat (copy), Debris (copy)
        local Target = p1.Target;
        local TowerPosition = p1.TowerPosition;
        local TargetPosition = p1.TargetPosition;
        local u2 = DiscoBall:Clone();
        ...
        if u2 then
            ...
            local u7 = nil;
            u7 = RunService.Heartbeat:Connect(function() -- Line: 45
                -- upvalues: u4 (copy), TowerPosition (copy), u6 (ref), TargetPosition (copy), u2 (copy), u7 (ref), DiscoBallHit (ref), Target (copy), ImpactGlass (ref), Beat (ref), Visuals (ref), Debris (ref)
                ...
                if v9 >= 1 then
                    ...
                    local v14 = ImpactGlass:Clone();
                    v14.Parent = v13;
                    v14:Play();
                    local v15 = Beat:Clone();
                    v15.Parent = v13;
                    v15:Play();
                    v13.Parent = Visuals;
