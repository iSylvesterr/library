...
local YellowEctoplasm = EffectAssets:WaitForChild("YellowEctoplasm");
local YellowEctoHit = EffectAssets:WaitForChild("YellowEctoHit");
...
return {
    Play = function(p1) -- Line: 21, Name: Play
        -- upvalues: YellowEctoplasm (copy), Visuals (copy), Spit (copy), Ghost (copy), RunService (copy), YellowEctoHit (copy), Splat (copy), Impact (copy), Debris (copy)
        local Target = p1.Target;
        local TowerPosition = p1.TowerPosition;
        local TargetPosition = p1.TargetPosition;
        local u2 = YellowEctoplasm:Clone();
        ...
        if u2 then
            ...
            u8 = RunService.Heartbeat:Connect(function() -- Line: 48
                -- upvalues: u5 (copy), TowerPosition (copy), u7 (ref), TargetPosition (copy), u2 (copy), u8 (ref), YellowEctoHit (ref), Target (copy), Splat (ref), Impact (ref), Visuals (ref), Debris (ref)
