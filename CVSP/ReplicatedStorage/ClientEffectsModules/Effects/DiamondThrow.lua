...
local Diamond = EffectAssets:WaitForChild("Diamond");
local DiamondHit = EffectAssets:WaitForChild("DiamondHit");
local DiamondHit2 = script:WaitForChild("DiamondHit");
...
return {
    Play = function(p1) -- Line: 20, Name: Play
        -- upvalues: Diamond (copy), Visuals (copy), Throw (copy), RunService (copy), DiamondHit (copy), DiamondHit2 (copy), Debris (copy)
        local Target = p1.Target;
        local TowerPosition = p1.TowerPosition;
        local TargetPosition = p1.TargetPosition;
        local u2 = Diamond:Clone();
        ...
        if u2 then
            ...
            u7 = RunService.Heartbeat:Connect(function() -- Line: 43
                -- upvalues: u4 (copy), TowerPosition (copy), u6 (ref), TargetPosition (copy), u2 (copy), u7 (ref), DiamondHit (ref), Target (copy), DiamondHit2 (ref), Visuals (ref), Debris (ref)
                ...
                if v9 >= 1 then
                    ...
                    if Attachment then
                        if Attachment:FindFirstChild("Sparkles") then
                            Attachment.Sparkles:Emit(16);
                        end;
