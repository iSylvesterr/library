...
-- [[ HASH SYSTEM ]] --
-- Nama GUI utama di-random tiap sesi (bukan "NapoleonOnTop" yang gampang ditebak),
-- dan di-parent ke RobloxGui (bukan langsung ke CoreGui) -- sama persis kayak NewUI.lua.
local function generateHash(length)
    -- Random.new() tanpa argumen di-seed otomatis pakai entropy internal Roblox --
    -- jauh lebih reliable buat "beda tiap execute" dibanding math.randomseed(tick()),
    -- yang gampang hasilin urutan mirip/sama kalau di-seed ulang di rentang waktu berdekatan.
    local chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
    local rng = Random.new()
    local hash = ""
    for i = 1, length do
        local rand = rng:NextInteger(1, #chars)
...
function Napoleon:Window(GuiConfig)
    ...
    function GuiFunc:DestroyGui()
        if RobloxGui:FindFirstChild(GUI_HASH) then
    ...
    Close.Activated:Connect(function()
        ...
        Yes.MouseButton1Click:Connect(function()
            ...
            if RobloxGui:FindFirstChild("ToggleUINapoleon") then
                RobloxGui.ToggleUINapoleon:Destroy()
            end
        end)
