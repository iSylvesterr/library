...
function u1.getIconToHighlight() -- Line: 134
    ...
    if not v13 then
        ...
        for _, v in pairs(iconsDictionary) do
            if not v.parentIconUID and (not v14 or v.widget.AbsolutePosition.X < v14) then
...
function u1.registerButton(u15) -- Line: 156
    -- upvalues: UserInputService (copy), GamepadService (copy), GuiService (copy)
    local u16 = false;
    u15.InputBegan:Connect(function(p17) -- Line: 162
        -- upvalues: u16 (ref)
        u16 = true;
        task.wait();
        task.wait();
    ...
    local u20 = UserInputService.InputBegan:Connect(function(p18) -- Line: 171
    ...
    u15.Destroying:Once(function() -- Line: 192
        -- upvalues: u20 (copy)
        u20:Disconnect();
    end);
end;
