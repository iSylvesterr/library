...
local u155 = {};
RunService.Heartbeat:Connect(function(p156) -- Line: 611
    -- upvalues: u155 (copy)
    for i, v in u155 do
        for i2, v2 in v do
            if v2:canSleep() then
...
local function assertType(p158, p159, p160, p161) -- Line: 631
    if not p160:find((typeof(p161))) then
        error(("bad argument #%d to %s (%s expected, got %s)"):format(p158, p159, p160, (typeof(p161))), 3);
    end;
end;

function v157.target(p162, p163, p164, p165) -- Line: 645
    -- upvalues: u155 (copy), u154 (copy)
    if not ("Instance"):find((typeof(p162))) then
        error(("bad argument #%d to %s (%s expected, got %s)"):format(1, "spr.target", "Instance", (typeof(p162))), 3);
    end;

    if not ("number"):find((typeof(p163))) then
        error(("bad argument #%d to %s (%s expected, got %s)"):format(2, "spr.target", "number", (typeof(p163))), 3);
    end;

    if not ("number"):find((typeof(p164))) then
        error(("bad argument #%d to %s (%s expected, got %s)"):format(3, "spr.target", "number", (typeof(p164))), 3);
    end;

    if not ("table"):find((typeof(p165))) then
        error(("bad argument #%d to %s (%s expected, got %s)"):format(4, "spr.target", "table", (typeof(p165))), 3);
    end;

    if p163 ~= p163 or p163 < 0 then
        error(("expected damping ratio >= 0; got %.2f"):format(p163), 2);
    end;

    if p164 ~= p164 or p164 < 0 then
        error(("expected undamped frequency >= 0; got %.2f"):format(p164), 2);
    end;

    local v166 = u155[p162];
    ...
    for i, v in p165 do
        ...
        if typeof(v) ~= typeof(v167) then
            error(("bad property %s to spr.target (%s expected, got %s)"):format(i, typeof(v167), (typeof(v))), 2);
        end;
        ...
        if p164 == (1 / 0) then
            ...
            if not v168 then
                local v169 = u154[typeof(v)];

                if not v169 then
                    error("unsupported type: " .. typeof(v), 2);
                end;

                v168 = v169.springType(p163, p164, v167, v, v169);
                v166[i] = v168;
            end;

            v168:setGoal(v);
            v168:setDampingRatio(p163);
            v168:setFrequency(p164);
        end;
    end;
...
function v157.stop(p170, p171) -- Line: 709
    -- upvalues: u155 (copy)
    if not ("Instance"):find((typeof(p170))) then
        error(("bad argument #%d to %s (%s expected, got %s)"):format(1, "spr.stop", "Instance", (typeof(p170))), 3);
    end;

    if not ("string|nil"):find((typeof(p171))) then
        error(("bad argument #%d to %s (%s expected, got %s)"):format(2, "spr.stop", "string|nil", (typeof(p171))), 3);
    end;

    if p171 then
        local v172 = u155[p170];
