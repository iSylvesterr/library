...
local function canOcclude(p39) -- Line: 197
    -- upvalues: u37 (ref), CollectionService (copy)
    local v40;
    ...
    if 1 - (1 - p39.Transparency) * (1 - p39.LocalTransparencyModifier) < 0.25 then
        ...
        if v40 then
            if u37 == (p39:GetRootPart() or p39) then
                ...
                v40 = not p39:IsA("TrussPart") and not CollectionService:HasTag(p39, "cameraIgnore");
            end;
        end;
    else
        v40 = false;
...
local function getCollisionPoint(p42, p43) -- Line: 226
    ...
    if u1 then
        ...
        repeat
            v44 = workspace:Raycast(p42, p43, u6);

            if v44 then
                if v44.Instance.CanCollide then
                    return v44.Position, true;
                end;

                u6:AddToFilter(v44.Instance);
            end;
        until not v44;
    else
        local v45 = #u26;
...
local function queryPoint(p50, p51, p52, p53) -- Line: 264
    -- upvalues: u26 (ref), u14 (ref), u1 (copy), u6 (copy), canOcclude (copy), u7 (copy), new (copy)
    ...
    if u1 then
        ...
        while true do
            ...
            if v59 >= 64 then
                v58 = v60;
                v60 = v57;
            elseif canOcclude(Instance) then
                ...
                if workspace:Raycast(v56, Position - v56, u7) then
                    ...
                    if p53 then
                        v63 = workspace:Raycast(p53, v56 - p53, u7) or workspace:Raycast(v56, p53 - v56, u7);
                    else
                        v63 = false;
                    end;
        ...
        while true do
            ...
            if true then
                local v66;
                v65, v66 = workspace:FindPartOnRayWithIgnoreList(new(p50, v56 - p50), u26, false, true);
                v59 = v59 + 1;

                if v65 then
                    local v67 = v59 >= 64;

                    if canOcclude(v65) or v67 then
                        local v68 = { v65 };
                        local v69 = workspace:FindPartOnRayWithWhitelist(new(v56, v66 - v56), v68, true);
                        v60 = (v66 - v64).Magnitude;

                        if v69 and not v67 then
                            local v70;

                            if p53 then
                                v70 = workspace:FindPartOnRayWithWhitelist(new(p53, v56 - p53), v68, true) or workspace:FindPartOnRayWithWhitelist(new(v56, p53 - v56), v68, true);
                            else
                                v70 = false;
                            end;
...
local function queryViewport(p72, p73) -- Line: 367
    -- upvalues: u5 (ref), u2 (copy), u4 (copy), u12 (ref), u13 (ref), u14 (ref), queryPoint (copy)
    debug.profilebegin("queryViewport");
    local p = p72.p;
    local rightVector = p72.rightVector;
    local upVector = p72.upVector;
    local v74 = -p72.lookVector;
    local v75;
    ...
    for i = 0, 1 do
        local v78 = rightVector * ((i - 0.5) * u12);

        for i2 = 0, 1 do
            local v79, v80 = queryPoint(p + u14 * (v78 + upVector * ((i2 - 0.5) * u13)), v74, p73, u5:ViewportPointToRay(ViewportSize.x * i, ViewportSize.y * i2).Origin);
...
local function testPromotion(p81, p82, p83) -- Line: 410
    ...
    local Magnitude = (getCollisionPoint(p, p83.posVelocity * 1.25) - p).Magnitude;

    for i = 0, min(1.25, p83.rotVelocity.magnitude + Magnitude / p83.posVelocity.magnitude), 0.0625 do
        local v85 = p83.extrapolate(i);

        if p82 <= queryPoint(v85.p, -v85.lookVector, p82) then
            return false;
        end;
    end;
    ...
    for _, v in ipairs(u41) do
        local v86 = getCollisionPoint(p, rightVector * v.x + upVector * v.y);

        if queryPoint(v86, (p + v84 * p82 - v86).Unit, p82) == (1 / 0) then
            return false;
        end;
    end;
...
return function(p87, p88, p89) -- Line: 459, Name: Popper
