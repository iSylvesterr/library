...
local function _posOffsetFrameDelta(p1, p2) -- Line: 15
    -- upvalues: Graph (copy), PartConstants (copy)
    if not p1.HasPosOffsetGraphs then
        return Vector3.new(0, 0, 0);
    end;

    local v3 = p1._staticPosOffsetX or (p1.Graphs.PosOffsetX and (Graph.QueryPointsWithTime(p2, p1.Graphs.PosOffsetX, p1.Seeds.PosOffsetX) or 0) or 0);
    local v4 = p1._staticPosOffsetY or (p1.Graphs.PosOffsetY and (Graph.QueryPointsWithTime(p2, p1.Graphs.PosOffsetY, p1.Seeds.PosOffsetY) or 0) or 0);
    local v5 = p1._staticPosOffsetZ or (p1.Graphs.PosOffsetZ and (Graph.QueryPointsWithTime(p2, p1.Graphs.PosOffsetZ, p1.Seeds.PosOffsetZ) or 0) or 0);
    local v6 = PartConstants.resolveDisplacement(Vector3.new(v3, v4, v5), p1.DisplacementMode or "Global", p1.SpawnRotation, p1.SpawnEmitterRotation, p1._displacementMirrorX, p1._displacementMirrorY, p1._displacementMirrorZ);
    local v7 = v6 - p1._prevWorldOff;
    p1._prevWorldOff = v6;

    return v7;
end;

local function getLinkCF(p8) -- Line: 30
...
return function(p10) -- Line: 45
    ...
    function p10.UpdateModel(p11, p12, p13, p14) -- Line: 50
