...
function u1.emitTargetInstance(u111, u112, p113, p114, p115, u116, p117) -- Line: 714
    ...
    task.delay(v126, function() -- Line: 791
...
function u1.fire(u127, u128, p129, p130, p131) -- Line: 800
    ...
    if u133.Module then
        function u137.Emit(p138, p139) -- Line: 831
        ...
        function u137.SetColor(p140) -- Line: 841
            -- upvalues: EventsPayload (ref), u128 (copy)
            EventsPayload.applyColor(u128, p140);
        end;

        function u137.SetTransparency(p141) -- Line: 842
            -- upvalues: EventsPayload (ref), u128 (copy)
            EventsPayload.applyTransparency(u128, p141);
        end;

        function u137.Teleport(p142) -- Line: 843
            -- upvalues: EventsPayload (ref), u128 (copy)
        ...
        function u137.SetSpeedMultiplier(p143) -- Line: 845
            -- upvalues: u128 (copy)
        ...
        function u137.SetLifetime(p144) -- Line: 850
            -- upvalues: u128 (copy)
    ...
    local v145 = #u127.ActiveEmits + (u127._lingerVisualCount or 0) >= (u127.MAX_ACTIVE_PARTICLES or 1000);

    if v145 and (u133.EmitTarget and not u1._capSkipWarned) then
        u1._capSkipWarned = true;
        warn("[Part-Icles Events] active-particle cap reached; event EmitTargets are being skipped");
    end;

    if u133.EmitTarget and not v145 then
        u1.emitTargetInstance(u127, u133.EmitTarget, u133.EmitMode, u137, u128, u132, u136);
    end;

    local u146 = u133.Module and u1.compile(u133.Module);

    if u146 then
        task.spawn(function() -- Line: 869
            -- upvalues: u146 (copy), u137 (ref), u1 (ref), u133 (copy)
            local v147, v148 = xpcall(u146, debug.traceback, u137);

            if not v147 then
                u1.reportScriptError(u133.Module, v148);
            end;
        end);
    end;
end;

function u1.afterUpdate(p149, p150, p151, p152) -- Line: 884
    -- upvalues: u1 (copy), EventsCollision (copy)
    if not (p150.Events and p150.Events.OnHit) then
        return;
    end;

    if p150._ownsOnHit then
        return;
    end;

    if p150._lastEffectiveDt and p150._lastEffectiveDt < 0 then
        p150.LastHitCheckPos = u1.getWorldPosition(p150);
        p150.LastHitCheckTime = p152;

        return;
    end;

    local v153 = u1.getWorldPosition(p150);

    if not v153 then
        return;
    end;

    if not p150.LastHitCheckPos then
        p150.LastHitCheckPos = v153;
        p150.LastHitCheckTime = p152;

        return;
    end;

    local v154 = p150.Events.OnHit.HitCheckInterval or 0;

    if v154 > 0 and p152 - (p150.LastHitCheckTime or 0) < v154 then
        return;
    end;

    local LastHitCheckPos = p150.LastHitCheckPos;
    local v155 = p152 - (p150.LastHitCheckTime or p152);
    local v156 = v155 <= 0 and 0.016666666666666666 or v155;
    local v157 = v153 - LastHitCheckPos;

    if v157.Magnitude > 0.05 then
        local v158 = workspace:Raycast(LastHitCheckPos, v157, p150.HitParams);

        if v158 then
            if not p150._hitFired then
                local v159 = u1.makePayload(p149, p150, "OnHit", nil);
                v159.HitInstance = v158.Instance;
                v159.Other = v158.Instance;
                v159.HitPosition = v158.Position;
                v159.HitNormal = v158.Normal;
                u1.fire(p149, p150, "OnHit", p150.EventChainCtx, v159);

                if EventsCollision.handle(p149, p150, v158, v157, v156) == "snap" then
                    return;
                end;
            end;
        elseif not p150._collisionStopped then
            p150._hitFired = false;
        end;
