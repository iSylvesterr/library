...
local function SafeUnit(p1) -- Line: 23
...
local function QueryTransformedWorldCFrameNonSmartbone(p4) -- Line: 95
...
local function QueryTransformedWorldCFrame(p8, p9) -- Line: 122
...
local function ClipVector(p11, p12, p13) -- Line: 144
    return p11 * (Vector3.new(1, 1, 1) - p13) + p12 * p13;
end;

local function GetFriction(p14, p15) -- Line: 150
...
local function SolveWind(p16, u17, p18) -- Line: 163
    ...
    local function EaseInExpo(p24) -- Line: 211
        return p24 == 0 and 0 or 2 ^ (p24 * 10 - 10);
    end;

    local v25 = p18.Magnitude < 100 and p18.Magnitude or 100;
    local v26 = math.min(v25 == 0 and 0 or 2 ^ (v25 * 10 - 10), 100);
    ...
    local function GetNoise(p31, p32, p33, p34) -- Line: 228
    ...
    local function SampleGust() -- Line: 240
        -- upvalues: u27 (ref)
        return math.sin(u27 * 1) * 0.3 + 0.7;
    end;

    local function SampleSin() -- Line: 246
    ...
    local function SampleNoise(p46, p47) -- Line: 264
...
function u57.new(u58, p59, p60) -- Line: 440
    ...
    u63.AttributeConnection = u58.AttributeChanged:Connect(function(p64) -- Line: 493
...
function u57.ClipVelocity(p65, p66, p67) -- Line: 510
    p65.LastPosition = p65.LastPosition * (Vector3.new(1, 1, 1) - p67) + p66 * p67;
end;

function u57.PreUpdate(p68, p69) -- Line: 516
    -- upvalues: QueryTransformedWorldCFrameNonSmartbone (copy), QueryTransformedWorldCFrame (copy)
...
function u57.StepPhysics(p75, p76, p77, p78) -- Line: 575
    ...
    if p75.Force or p75.Gravity then
        p77 = (p75.Gravity or p76.Settings.Gravity) + (p75.Force or p76.Settings.Force);
    end;

    local Settings = p76.Settings;
    local v79 = (p75.Position - p75.LastPosition) / p78;

    if v79.Magnitude > 50 then
        v79 = v79.Unit * 50 or v79;
    end;

    local v80 = p76.ObjectAcceleration * Settings.Inertia;
    local v81 = SolveWind(p75, p76, v79);
    p75.LastPosition = p75.Position;
    p75.Position = p75.Position + (v79 * (1 - Settings.Damping) * p78 + (p77 + v80) * p78 * p78 + v81);
end;

function u57.Constrain(p82, p83, p84, p85) -- Line: 615
...
function u57.SkipUpdate(p90) -- Line: 671
...
function u57.SolveTransform(p91, p92, p93) -- Line: 686
...
function u57.ApplyTransform(p97, p98) -- Line: 725
    ...
    if v99 and Bone then
        if v99.Anchored and not p98.Settings.AnchorsRotate then
            Bone.WorldCFrame = v99.TransformOffset;

            return;
        end;

        if v99.Anchored then
            Bone.WorldCFrame = CFrame.new(v99.Position) * v99.CalculatedWorldCFrame.Rotation;
...
function u57.DrawDebug(p100, p101, p102, p103, p104, p105, p106) -- Line: 766
    -- upvalues: Gizmo (copy)
...
function u57.DrawOverlay(p138, p139) -- Line: 957
    -- upvalues: Config (copy)
...
function u57.Destroy(p142) -- Line: 993
    -- upvalues: Config (copy)
