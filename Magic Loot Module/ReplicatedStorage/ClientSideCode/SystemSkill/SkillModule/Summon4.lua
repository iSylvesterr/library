...
u1.summonMaxCount = 1;
u1.summonSkillKey = "Summon4";

local function cleanupSummonTrail(p2) -- Line: 50
    if not p2 then
        return;
    end;
...
local function playTargetBurstVfx(u4, u5) -- Line: 70
    -- upvalues: FXUtil (copy)
    local function doBurstEmit() -- Line: 71
    ...
    if u6 then
        ...
        task.delay(0.6, function() -- Line: 88
...
local function disableSphereFlyVfx(p8) -- Line: 111
    -- upvalues: FXUtil (copy)
...
function u1.Client_EnterStartup(u9) -- Line: 171
    ...
    local function stillTrail() -- Line: 182
    ...
    task.delay(0.3, function() -- Line: 187
        ...
        SkillCommon.connectRunEventWhile(skillRunData, "召唤术魔杖尾迹", RunService.RenderStepped, stillTrail, function() -- Line: 206
    ...
    task.delay(0, function() -- Line: 214
        ...
        SkillCommon.playSoundLocal3D("音效-召唤法术-通用法阵", v17:GetPivot().Position);
    end);
    task.delay(1.4, function() -- Line: 233
...
function u1.Server_EnterStartup(p18) -- Line: 239
end;

function u1.Client_EnterSummon(u19) -- Line: 244
    -- upvalues: SkillCommon (copy), playFormationL4Vfx (copy), FXUtil (copy), RunService (copy), disableSphereFlyVfx (copy), playTargetBurstVfx (copy)
    ...
    local function stillSummon() -- Line: 258
    ...
    local function scheduleFormationL4() -- Line: 275
        -- upvalues: u19 (copy), runGeneration (copy), material (copy), u20 (copy), Position (copy), playFormationL4Vfx (ref)
        task.delay(0.4, function() -- Line: 276
            -- upvalues: u19 (ref), runGeneration (ref), material (ref), u20 (ref), Position (ref), playFormationL4Vfx (ref)
            ...
            local v27 = material["召唤法阵4级_Emit和Enable"];
    ...
    local u31 = 0;
    local u32 = false;
    SkillCommon.connectRunEventWhile(skillRunData, "召唤空间球飞行", RunService.Heartbeat, stillSummon, function(p33) -- Line: 315
        -- upvalues: u31 (ref), u29 (copy), u30 (copy), u25 (copy), u32 (ref), SkillCommon (ref), skillRunData (copy), disableSphereFlyVfx (ref), material (copy), u20 (copy), playTargetBurstVfx (ref), scheduleFormationL4 (copy), u19 (copy), runGeneration (copy), Position (copy), playFormationL4Vfx (ref)
        u31 = u31 + p33;
        ...
        if not v35 then
            task.delay(0.4, function() -- Line: 276
                ...
                local v37 = material["召唤法阵4级_Emit和Enable"];
        ...
        playTargetBurstVfx(v35, scheduleFormationL4);
        SkillCommon.playSoundLocal3D("音效-召唤法术-通用融合", u25.Position);
        SkillCommon.playSoundLocal3D("音效-召唤法术-召唤物的法阵圈4", u25.Position);
        task.delay(3, function() -- Line: 341
            -- upvalues: SkillCommon (ref), u25 (ref)
            SkillCommon.playSoundLocal3D("音效-召唤法术-通用法阵圈消散", u25.Position);
        end);
    end);
end;

function u1.Client_ExitSummon(p38) -- Line: 350
...
function u1.Server_EnterSummon(u40) -- Line: 363
    ...
    task.delay(1, function() -- Line: 383, Name: doSummon
        -- upvalues: u40 (copy), UtilsSystem (ref), u43 (copy), u1 (ref), u41 (copy)
        ...
        local v44 = game.Players:GetPlayerFromCharacter(character2);

        if not v44 then
            return;
        end;

        UtilsSystem.SystemSummon.CreateSummon(v44, 20000004, u43, {
            spawnGroundOffsetY = 6,
            spawnIntroDuration = 3,
            spawnIntroBelowOffsetY = 10,
...
function u1.Server_ExitSummon(p45) -- Line: 417
end;

function u1.Server_EnterRecovery(p46) -- Line: 422
    p46:releaseControl();
end;

function u1.Client_EnterRecovery(p47) -- Line: 426
end;

function u1.onEnd(p48) -- Line: 430
...
u1.SoundList = { "音效-召唤法术-通用融合", "音效-召唤法术-通用法阵圈消散", "音效-召唤法术-通用法阵", "音效-召唤法术-通用投掷", "音效-召唤法术-召唤物的法阵圈4" };
u1.AnimateList = { "小召唤术" };
u1.ResNameList = { "召唤法阵A01_起手空间法阵_Emit", "召唤法阵A02_起手空间爆点_Emit", "召唤法阵A03_空间球_Enable", "召唤法阵A04_空间爆发_Emit", "召唤法阵4级_Emit和Enable", "召唤系尾迹" };
u1.hitboxConfig = {};
u1.Action = {
    {
        action = "Animation",
