...
local function get_skillScale(p4) -- Line: 86
...
local function resolveThrowArm(p10) -- Line: 94
    return p10:FindFirstChild("Right Arm") or p10:FindFirstChild("RightHand");
end;

local function buildThrowPath(p11, p12, p13) -- Line: 100
    ...
    if p13 and p13.Parent then
        v14 = p13:GetPivot();
    elseif p11 then
        local v15 = p11:FindFirstChild("Right Arm") or p11:FindFirstChild("RightHand");
...
local function evalThrowFlightCF(p20, p21, p22, p23) -- Line: 131
    local v24 = p20.Position:Lerp(p21.Position, p23);

    return CFrame.lookAt(v24, v24 + p22);
end;

local function buildStopCFrame(p25, p26, p27) -- Line: 136
...
local function isCharacterObstaclePart(p28) -- Line: 147
    if p28 then
        p28 = p28.Parent;
    end;
...
local function releaseProjectileWelds(p30) -- Line: 155
...
local function detachThrownProjectileVisual(p32) -- Line: 172
...
local function scheduleObstacleHoldFade(u33) -- Line: 181
...
local function freezeThrownVisualAtCurrent(u35) -- Line: 197
    -- upvalues: releaseProjectileWelds (copy), FXUtil (copy)
...
local function tryClientObstacleStop(u37, p38, p39, p40, p41) -- Line: 205
    -- upvalues: releaseProjectileWelds (copy), FXUtil (copy)
    if u37.stopped then
        return true;
    end;
...
local function resolveStickCF(p47, p48) -- Line: 237
...
local function finalizeThrownHitStop(u49, p50) -- Line: 250
...
local function disconnectThrownMoveEvent(p53) -- Line: 266
...
local function handleClientHitStop(p54, p55, p56) -- Line: 273
...
local function cleanupThrownProjectileVisual(p57) -- Line: 281
    ...
    local v59 = skillRunData.material and skillRunData.material["投掷物"];
...
function v3.Client_EnterStartup(p60) -- Line: 300
    ...
    local u61 = character:FindFirstChild("Right Arm");

    if not u61 then
        return;
    end;

    local u62 = p60.skillRunData.material["投掷物"];
    u62.Parent = workspace.Debris;
    p60.skillRunData.runEvent["投掷物跟手"] = RunService.Heartbeat:Connect(function(p63) -- Line: 313
        -- upvalues: u61 (copy), u62 (copy), u2 (ref)
        if u61 and u61.Parent ~= nil then
            u62:PivotTo(u61:GetPivot():ToWorldSpace(u2));
        end;
    end);
    task.delay(0.1, function() -- Line: 320
...
function v3.Server_EnterStartup(p64) -- Line: 329
...
function v3.Client_EnterThrownMoving(u66) -- Line: 335
    ...
    local u67 = u66.skillRunData.material["投掷物"];
    ...
    u66.skillRunData.runEvent["投掷物移动"] = RunService.Heartbeat:Connect(function(p73) -- Line: 374
        ...
        u72 = u72 + p73;
        local v74 = game.TweenService:GetValue(math.clamp(u72 / 0.75, 0, 1), Enum.EasingStyle.Quad, Enum.EasingDirection.Out);
...
function v3.Client_ExitThrownMoving(p79) -- Line: 406
end;

function v3.Server_EnterThrownMoving(u80) -- Line: 413
    ...
    hitbox:PivotTo(u82);
    local u85 = 0;
    u80.skillRunData.runEvent["投掷物伤害盒移动"] = RunService.Heartbeat:Connect(function(p86) -- Line: 439
        ...
        u85 = u85 + p86;
        local v87 = game.TweenService:GetValue(math.clamp(u85 / 0.75, 0, 1), Enum.EasingStyle.Quad, Enum.EasingDirection.Out);
...
function v3.Server_ExitThrownMoving(p89) -- Line: 460
...
function v3.Server_EnterRecovery(p91) -- Line: 470
    p91:releaseControl();
end;

function v3.Client_EnterRecovery(p92) -- Line: 474
end;

function v3.Server_UpdateProjectileObstacleCheck(p93) -- Line: 478
...
function v3.onServerEvent(p101, p102) -- Line: 529
...
function v3.onEnd(p103) -- Line: 551
    -- upvalues: cleanupThrownProjectileVisual (copy)
    cleanupThrownProjectileVisual(p103);
end;

v3.SoundList = { "音效-技能-剑-whoosh" };
v3.AnimateList = { "人形生物投掷1" };
v3.ResNameList = { "投掷物" };
v3.hitboxConfig = { {
        HitboxIndex = 1,
        PartName = "通用长方体",
