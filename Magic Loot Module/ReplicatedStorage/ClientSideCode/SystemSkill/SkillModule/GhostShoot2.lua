    ...
    for _, v in u14 do
        for i = 1, 3 do
            ...
            if material then
                ...
                material = skillRunData.material[v95];
            end;
...
local function resolveShotPointWorldPos(p96) -- Line: 514
    if not p96 then
        return nil;
    end;

    local v97 = p96:FindFirstChild("当前手持");

    if not v97 then
        return nil;
    end;

    local ShotPoint = v97:FindFirstChild("ShotPoint");
    ...
    if ShotPoint:IsA("BasePart") then
        v98 = ShotPoint.Position;
    elseif ShotPoint:IsA("Model") then
        v98 = ShotPoint:GetPivot().Position;
    elseif ShotPoint:IsA("Attachment") then
        v98 = ShotPoint.WorldPosition;
    end;

    if v98 then
        return v98 + Vector3.new(0, 4, 0);
    end;

    return nil;
end;

local function buildShotTrajectory(p99, p100, p101, p102) -- Line: 615
    -- upvalues: u2 (copy)
    local v103 = u2[p99] or u2[1];
    local v104 = {
        spiralUpLiftStuds = 16,
        spiralEndMinDist = (v103.spiralEndMinDist or 25) * p102,
        spiralNearExpandStuds = v103.spiralNearExpandStuds * p102,
        finalStraightStuds = v103.finalStraightStuds * p102,
        spiralAmp = math.clamp((p101 - p100).Magnitude * 0.12 * (v103.spiralAmpScale or 1), 3, 40),
        flightSec = v103.flightSec,
        easingStyle = v103.easingStyle,
        easingDirection = v103.easingDirection
    };
    local v105 = (u2[p99] or u2[1]).spiralPhaseOffsetDeg or 0;
    v104.spiralPhaseOffsetRad = math.abs(v105) < 0.0001 and 0 or math.rad(v105);

    return v104;
end;

local function playGroundExplosionFx(p106, p107, p108, p109, p110) -- Line: 552
    -- upvalues: FXUtil (copy), VisibleMgr (copy), SkillCommon (copy)
    local v111 = p106.material[p107];

    if not v111 then
        return;
    end;

    local v112 = FXUtil.GetGroundAlignedCF(p108, p109, "Ground", 3, 0.1) or CFrame.new(p108 + Vector3.new(0, 0.1, 0));
    VisibleMgr.UnQueryAll(v111);
    v111:ScaleTo(p110);
    v111:PivotTo(v112);
    v111.Parent = workspace.Debris;
    FXUtil.Emit_Particles_GetDescendants(v111, true);
    SkillCommon.appendRunSpawnList(p106, "ghostShootSpawns", v111);
end;

local function buildMuzzleCF(p113) -- Line: 540
    return CFrame.lookAt(p113, p113 + Vector3.new(0, 1, 0));
end;

local function cleanupServerOnInterrupt(p114) -- Line: 312
    ...
    cleanupRunFx(p114, true);
    local v115 = p114.hitbox[1];

    if v115 and v115.isActive then
        v115:stop();
    end;

    local v116 = p114.hitbox[2];

    if v116 and v116.isActive then
        v116:stop();
    end;

    local v117 = p114.hitbox[3];

    if v117 and v117.isActive then
        v117:stop();
    end;
end;

local function resolveShotSnapEnd(p118, p119, p120, p121) -- Line: 584
    -- upvalues: u2 (copy)
    local v122 = ((u2[p120] or u2[1]).aimOffsetRight or 0) * p121;

    if math.abs(v122) < 0.0001 then
        return p119;
    end;

    local HumanoidRootPart = p118:FindFirstChild("HumanoidRootPart");
    ...
    local v123 = Vector3.new(p119.X - Position.X, 0, p119.Z - Position.Z);

    if v123.Magnitude < 0.0001 then
        local LookVector = HumanoidRootPart.CFrame.LookVector;
        v123 = Vector3.new(LookVector.X, 0, LookVector.Z);
    end;
    ...
    return p119 + Vector3.new(-v123.Z, 0, v123.X).Unit * v122;
end;

local function cloneShotMaterials(p124) -- Line: 569
    -- upvalues: u14 (copy)
    for _, v in u14 do
        local v125 = p124.material[v];
        ...
        if v125 then
            for i = 2, 3 do
                ...
                if not p124.material[v126] then
                    p124.material[v126] = v125:Clone();
                end;
            end;
        end;
    end;
...
local function sampleProjectileMotion(p127, p128, p129) -- Line: 473
    -- upvalues: MathMgr (copy), TweenService (copy), ensureBezierEndState (copy), cubicBezierTangent (copy)
    local snapEnd0 = p127.snapEnd0;
    local v130 = p127.spiralPhaseOffsetRad or 0;
    local SPIRAL_FIB_SPIRAL_U_PORTION = MathMgr.SPIRAL_FIB_SPIRAL_U_PORTION;
    local v131 = TweenService:GetValue(p128, p127.easingStyle, p127.easingDirection);

    if v131 < SPIRAL_FIB_SPIRAL_U_PORTION - 0.0001 then
        return MathMgr.spiralFibLikeChordPosTangent(p127.skullStart, snapEnd0, v131, p127.spiralAmp, p127.spiralEndMinDist, p127.finalStraightStuds, p127.spiralNearExpandStuds, v130, p127.spiralUpLiftStuds);
    end;

    ensureBezierEndState(p127, p129);
    local bezierP0 = p127.bezierP0;
    local bezierP1 = p127.bezierP1;
    local v132 = p129 - bezierP0;
    local v133;

    if v132.Magnitude > 0.0001 then
        v133 = v132.Unit;
    else
        v133 = (bezierP1 - bezierP0).Unit;
    end;

    local v134 = p129 - (v133.Magnitude < 0.0001 and Vector3.new(0, 0, -1) or v133) * p127.bezierArmLen;

    if p127.easingStyle == Enum.EasingStyle.Quad and p127.easingDirection == Enum.EasingDirection.In then
        SPIRAL_FIB_SPIRAL_U_PORTION = math.sqrt(SPIRAL_FIB_SPIRAL_U_PORTION);
    end;

    local v135 = (p128 - SPIRAL_FIB_SPIRAL_U_PORTION) / math.max(1 - SPIRAL_FIB_SPIRAL_U_PORTION, 1e-6);
    local v136 = TweenService:GetValue(math.clamp(v135, 0, 1), Enum.EasingStyle.Quad, Enum.EasingDirection.In);
    local v137 = math.clamp(v136, 0, 1);
    local v138 = 1 - v137;

    return v138 * v138 * v138 * bezierP0 + v138 * 3 * v138 * v137 * bezierP1 + v138 * 3 * v137 * v137 * v134 + v137 * v137 * v137 * p129, cubicBezierTangent(bezierP0, bezierP1, v134, p129, v136);
end;
...
local function fireClientShot(u179, p180) -- Line: 849
    -- upvalues: SkillCommon (copy), resolveShotPointWorldPos (copy), resolveShotSnapEnd (copy), buildShotTrajectory (copy), SkillTelegraph (copy), VisibleMgr (copy), buildMuzzleCF (copy), FXUtil (copy), MathMgr (copy), u13 (copy), ensureClientProjectileList (copy), u93 (copy), RunService (copy), doClientImpact (copy), sampleProjectileMotion (copy), cleanupRunFx (copy)
    ...
    if skillRunData2.runEvent[u93.clientMotion] then
        return;
    end;

    skillRunData2.runEvent[u93.clientMotion] = RunService.Heartbeat:Connect(function(p197) -- Line: 793
...
local function fireServerShot(u251, p252) -- Line: 1122
    -- upvalues: resolveShotPointWorldPos (copy), SkillCommon (copy), resolveShotSnapEnd (copy), buildShotTrajectory (copy), ensureServerProjectileList (copy), u93 (copy), RunService (copy), sampleProjectileMotion (copy), doServerImpact (copy), cleanupRunFx (copy)
    ...
    if skillRunData.runEvent[u93.serverMotion] then
        return;
    end;

    skillRunData.runEvent[u93.serverMotion] = RunService.Heartbeat:Connect(function(p259) -- Line: 1085
