...
local function segmentPlanKey(p3) -- Line: 86
    return "multThunderSegPlan" .. p3;
end;

local function segmentLandFxKey(p4) -- Line: 90
    return "multThunderLandFxDone" .. p4;
end;

local function segmentLandPlayerStrikeKey(p5) -- Line: 94
...
local function applyHitboxVisibility(p7, p8) -- Line: 255
    if not p7 then
        return;
    end;
...
local function snapGroundPos(p9) -- Line: 266
    -- upvalues: SkillCommon (copy)
    return SkillCommon.getGroundCF(CFrame.new(p9), 4, 0.15, "Ground").Position;
end;

local function withLandingYOffset(p10) -- Line: 270
    return Vector3.new(p10.X, p10.Y + 0.1, p10.Z);
end;

local function snapLandingGroundPos(p11) -- Line: 275
    -- upvalues: SkillCommon (copy)
...
local function flatDistance(p12, p13) -- Line: 279
    return (Vector3.new(p12.X, 0, p12.Z) - Vector3.new(p13.X, 0, p13.Z)).Magnitude;
end;

local function buildPathLeapSegmentTimings(p14) -- Line: 285
...
local function getFlatLookHint(p21) -- Line: 299
...
local function strikeFlatHint(p23, p24) -- Line: 310
...
local function emitJumpStrikeWarningAt(p35, p36, p37, p38) -- Line: 339
...
local function emitJumpStrikeAt(p39, p40, p41, p42, p43, p44) -- Line: 344
...
local function pulseJumpStrikeHitboxAtGround(u46, p47, p48, p49, p50) -- Line: 360
    ...
    u46:start();
    task.delay(p49 or 0.15, function() -- Line: 376
...
local function stillActiveForScheduledStrike(p52, p53) -- Line: 385
    return p52.runGeneration == p53;
end;

local function scheduleJumpStrikeClient(u54, u55, u56, u57, p58) -- Line: 389
    ...
    local u60 = v59["独角兽落雷预警"];
    local u61 = v59["独角兽落雷"];
    local u62 = v59["独角兽落雷地面特效"];
    ...
    task.delay(p58, function() -- Line: 408
    ...
    task.delay(p58 + 1.2, function() -- Line: 414
...
local function scheduleJumpStrikeServer(u65, u66, u67, u68, p69, p70) -- Line: 422
    -- upvalues: pulseJumpStrikeHitboxAtGround (copy)
    local u71 = p70 or 2;
    task.delay(p69 + 1.2, function() -- Line: 431
...
local function isLandingPlayerStrikeTarget(p73, p74, p75) -- Line: 443
...
local function gatherNearbyPlayerLandingStrikePositions(p80, p81, p82) -- Line: 468
    ...
    table.sort(v83, function(p85, p86) -- Line: 489
...
local function strikeFlatHintFromCenter(p88, p89) -- Line: 502
    local v90 = p89 - p88;
...
local function scheduleLandingPlayerStrikesAtPositions(u92, p93, p94, u95, p96, p97) -- Line: 511
    ...
    for i, v in p93 do
        ...
        if p96 then
            ...
            task.delay(p97 + 1.2, function() -- Line: 431
...
local function scheduleLandingPlayerStrikesIfNeeded(p104, p105, p106, p107) -- Line: 535
...
local function scheduleExtraPathStrikes(u109, p110, u111, p112) -- Line: 558
    ...
    for i = 1, 3 do
        ...
        for i2 = 1, v117 do
            ...
            if p112 then
                local u123 = 2;
                task.delay(v122 + 1.2, function() -- Line: 431
...
local function emitFootFxModel(p125, p126, p127) -- Line: 594
...
local function resolveGroundAlignedFootCF(p128, p129, p130) -- Line: 605
    -- upvalues: FXUtil (copy)
    if not p129 then
...
local function playJumpFootFx(p131, p132, p133) -- Line: 614
    ...
    local v135 = skillRunData.material and skillRunData.material["雷跃起跳"];
...
local function landFxWorldPosFromPlan(p142) -- Line: 636
    return Vector3.new(p142.endPos.X, p142.endGroundPos.Y, p142.endPos.Z);
end;

local function segmentMovementFlatHint(p143) -- Line: 640
...
local function playLandFootFx(p145, p146, p147) -- Line: 653
    ...
    local v149 = skillRunData.material and skillRunData.material["雷跃落地"];
...
local function playSegmentLandFootFxIfNeeded(p154, p155, p156, p157, p158) -- Line: 679
...
local function onSegmentLandingIfNeeded(p163, p164, p165, p166, p167) -- Line: 704
...
local function storeSegmentPlan(p168, p169) -- Line: 715
    p168.Logic["multThunderSegPlan" .. p169.segmentIndex] = p169;
end;

local function getStoredSegmentPlan(p170, p171) -- Line: 719
    return p170.Logic["multThunderSegPlan" .. p171];
end;

local function disconnectRunEvent(p172, p173) -- Line: 723
    -- upvalues: SkillCommon (copy)
    SkillCommon.disconnectRunEventKeys(p172.skillRunData, { p173 });
end;

local function buildFlatLookRotation(p174, p175, p176) -- Line: 727
...
local function resolveMoveFaceMode(p178) -- Line: 735
    -- upvalues: u2 (copy)
    return p178 and p178.moveFaceMode == u2.MoveFaceMode.AttackTarget and "AttackTarget" or "MoveTarget";
end;

local function resolveSegmentTravelRotation(p179, p180) -- Line: 742
...
local function resolveSegmentFaceRotation(p181, p182, p183) -- Line: 749
    -- upvalues: buildFlatLookRotation (copy)
    local v184;
...
local function resolveRawGroundY(p187) -- Line: 758
    -- upvalues: SkillCommon (copy)
    return SkillCommon.getGroundCF(CFrame.new(p187), 4, 0, "Ground").Position.Y;
end;

local function resolveFrozenStrikeWorld(p188) -- Line: 763
    -- upvalues: SkillCommon (copy)
...
local function hasAuthorityPathPoints(p189) -- Line: 773
...
local function resolveCastSpawnGround(p190, p191, p192) -- Line: 777
...
local function pathPlanFromAuthorityPoints(p193) -- Line: 785
    -- upvalues: MultThunderTramplePath (copy)
    return MultThunderTramplePath.planFromPoints(p193);
end;

local function commitPathRootFromAuthority(p194, p195, p196, p197) -- Line: 789
...
local function publishMultThunderPathAuthority(p207, p208) -- Line: 842
    -- upvalues: MultThunderTramplePath (copy)
...
local function buildPathPlanOnceAtSkillStart(p209, p210, p211, p212) -- Line: 853
...
local function commitPathRoot(p215) -- Line: 868
...
local function resolveSegmentStartGround(p232, p233, p234, p235, p236) -- Line: 951
    -- upvalues: SkillCommon (copy)
...
local function recordSegmentEndGround(p237, p238) -- Line: 979
    p237.lastSegmentEndGround = p238;
end;

local function commitSegmentMovePlan(p239, p240, p241) -- Line: 983
...
local function sampleSegmentMoveCF(p249, p250, p251) -- Line: 1032
...
local function resolveRecoveryTargetRotation(p263, p264, p265) -- Line: 1046
    -- upvalues: buildFlatLookRotation (copy)
...
local function lockCharacterForSkillMove(p272) -- Line: 1056
...
local function getSkillCharacter(p275) -- Line: 1077
    return p275.character or p275.skillInputData and p275.skillInputData.character;
end;

local function resolveHrpAndHumanoid(p276, p277) -- Line: 1081
...
local function clearSkillMoveLockAttribute(p279) -- Line: 1095
...
local function markSkillMoveLockApplied(p280, p281) -- Line: 1102
...
local function syncNpcEntityPos(p282, p283) -- Line: 1109
...
local function serverStartSmoothFaceRotation(u285, u286, u287, u288, u289, u290, u291) -- Line: 1127
    ...
    skillRunData.runEvent[u286] = RunService.Heartbeat:Connect(function() -- Line: 1148
...
local function serverStartRecoveryFaceRotation(u300, u301, u302, u303, u304) -- Line: 1173
    ...
    skillRunData.runEvent = skillRunData.runEvent or {};
    skillRunData.runEvent.MultThunderTrampleRecoveryFace = RunService.Heartbeat:Connect(function() -- Line: 1193
...
local function pinCharacterAtSegmentEnd(p323, p324, p325) -- Line: 1230
...
local function pinCharacterAtSegmentPlanEnd(p328, p329, p330) -- Line: 1240
    -- upvalues: sampleSegmentMoveCF (copy), UtilsSystem (copy)
...
local function getSkillAnimator(p333) -- Line: 1251
    if not p333 then
        return nil;
    end;
...
local function setTrackLooped(p335, p336, p337) -- Line: 1262
...
local function stopSegmentSkillAnims(p338) -- Line: 1271
...
local function serverPlayLeapAnim(p339, p340) -- Line: 1279
...
local function serverStopLeapAnim(p344) -- Line: 1291
    -- upvalues: AnimationModule (copy)
...
local function serverPlayPauseWalkAnim(p348) -- Line: 1299
...
u299 = function(p352) -- Line: 1311, Name: serverStopPauseWalkAnim
    -- upvalues: AnimationModule (copy)
...
local function serverEnterPauseSegment(u356, p357) -- Line: 1320
    ...
    skillRunData.runEvent = skillRunData.runEvent or {};
    skillRunData.runEvent.MultThunderTramplePauseFace = RunService.Heartbeat:Connect(function() -- Line: 1362
...
local function serverExitPauseSegment(p372, p373) -- Line: 1381
...
local function applyHumanoidPhysicsAfterUnanchor(p378, p379) -- Line: 1397
...
local function releaseSkillMoveAnchoring(p380, p381, p382, p383, p384) -- Line: 1413
...
local function restoreCharacterMoveState(p390) -- Line: 1451
...
function u2.onEndServer(p396) -- Line: 1474
...
local function segmentMovementProgress(p402, p403) -- Line: 1501
    -- upvalues: ThunderLeapTiming (copy)
    return ThunderLeapTiming.sampleDisplacementU(p402.nowTime - p403.startedAt, p403.phases);
end;

local function applySegmentMovementSample(p404, p405, p406, p407) -- Line: 1506
...
local function tryFinishMovementSegment(p408, p409, p410, p411, p412) -- Line: 1514
...
local function beginLeapAtPoint(u413, p414, p415, u416, p417) -- Line: 1538
    ...
    if p417 then
        local u423 = 2;
        task.delay(1.2, function() -- Line: 431
...
local function ensurePathRoot(p425) -- Line: 1564
...
local function serverEnterMovementSegment(u427, u428) -- Line: 1578
    ...
    skillRunData.runEvent = skillRunData.runEvent or {};
    skillRunData.runEvent.MultThunderTrampleMove = RunService.Heartbeat:Connect(function() -- Line: 1647
...
local function serverExitMovementSegment(p454, p455) -- Line: 1695
...
local function clientEnterMovementSegment(u468, u469) -- Line: 1715
    ...
    skillRunData.runEvent = skillRunData.runEvent or {};
    skillRunData.runEvent.MultThunderTrampleMove = RunService.Heartbeat:Connect(function() -- Line: 1755
...
local function clientExitMovementSegment(p479, p480) -- Line: 1780
...
function u2.Client_EnterStartup(p483) -- Line: 1790
    -- upvalues: commitPathRoot (copy)
...
function u2.Client_ExitStartup(p485) -- Line: 1799
end;

function u2.Server_EnterStartup(p486) -- Line: 1802
...
function u2.Client_EnterMovement1(p494) -- Line: 1851
    -- upvalues: clientEnterMovementSegment (copy)
    clientEnterMovementSegment(p494, 1);
end;

function u2.Client_ExitMovement1(p495) -- Line: 1855
    -- upvalues: clientExitMovementSegment (copy)
    clientExitMovementSegment(p495, 1);
end;

function u2.Server_EnterMovement1(p496) -- Line: 1859
    -- upvalues: serverEnterMovementSegment (copy)
    serverEnterMovementSegment(p496, 1);
end;

function u2.Server_ExitMovement1(p497) -- Line: 1863
    -- upvalues: serverExitMovementSegment (copy)
    serverExitMovementSegment(p497, 1);
end;

function u2.Client_EnterPause1(p498) -- Line: 1867
end;

function u2.Client_ExitPause1(p499) -- Line: 1870
end;

function u2.Server_EnterPause1(p500) -- Line: 1873
    -- upvalues: serverEnterPauseSegment (copy)
    serverEnterPauseSegment(p500, 1);
end;

function u2.Server_ExitPause1(p501) -- Line: 1877
    -- upvalues: serverExitPauseSegment (copy)
    serverExitPauseSegment(p501, 1);
end;

function u2.Client_EnterMovement2(p502) -- Line: 1881
    -- upvalues: clientEnterMovementSegment (copy)
    clientEnterMovementSegment(p502, 2);
end;

function u2.Client_ExitMovement2(p503) -- Line: 1885
    -- upvalues: clientExitMovementSegment (copy)
    clientExitMovementSegment(p503, 2);
end;

function u2.Server_EnterMovement2(p504) -- Line: 1889
    -- upvalues: serverEnterMovementSegment (copy)
    serverEnterMovementSegment(p504, 2);
end;

function u2.Server_ExitMovement2(p505) -- Line: 1893
    -- upvalues: serverExitMovementSegment (copy)
    serverExitMovementSegment(p505, 2);
end;

function u2.Client_EnterPause2(p506) -- Line: 1897
end;

function u2.Client_ExitPause2(p507) -- Line: 1900
end;

function u2.Server_EnterPause2(p508) -- Line: 1903
    -- upvalues: serverEnterPauseSegment (copy)
    serverEnterPauseSegment(p508, 2);
end;

function u2.Server_ExitPause2(p509) -- Line: 1907
    -- upvalues: serverExitPauseSegment (copy)
    serverExitPauseSegment(p509, 2);
end;

function u2.Client_EnterMovement3(p510) -- Line: 1911
    -- upvalues: clientEnterMovementSegment (copy)
    clientEnterMovementSegment(p510, 3);
end;

function u2.Client_ExitMovement3(p511) -- Line: 1915
    -- upvalues: clientExitMovementSegment (copy)
    clientExitMovementSegment(p511, 3);
end;

function u2.Server_EnterMovement3(p512) -- Line: 1919
    -- upvalues: serverEnterMovementSegment (copy)
    serverEnterMovementSegment(p512, 3);
end;

function u2.Server_ExitMovement3(p513) -- Line: 1923
    -- upvalues: serverExitMovementSegment (copy)
    serverExitMovementSegment(p513, 3);
end;

function u2.Server_EnterRecovery(p514) -- Line: 1927
...
function u2.Server_ExitRecovery(p523) -- Line: 1949
...
function u2.Client_EnterRecovery(p524) -- Line: 1955
    -- upvalues: SkillCommon (copy)
    SkillCommon.disconnectRunEventKeys(p524.skillRunData, { "MultThunderTrampleMove" });
end;

function u2.onEnd(p525) -- Line: 1959
    -- upvalues: SkillCommon (copy)
...
u2.ResNameList = { "雷跃起跳", "雷跃落地", "独角兽落雷地面特效", "独角兽落雷", "独角兽落雷预警" };
u2.hitboxConfig = {};
...
function u2.estimateSkillTotalDuration(p526) -- Line: 2007
