...
local u2 = CFrame.new(0, 0, -4) * CFrame.Angles(1.5707963267948966, 0, 0);
...
function u1.Client_EnterStartup(u10) -- Line: 130
    ...
    task.delay(0.27, function() -- Line: 139
        -- upvalues: u10 (copy), RunService (ref), u11 (copy)
        if not u10:isRunningFlow() then
            return;
        end;

        local u12 = u10.skillRunData.material["冰系尾迹"];
        ...
        u10.skillRunData.runEvent["冰锥术Cast尾迹"] = RunService.RenderStepped:Connect(function() -- Line: 148
...
function u1.Server_EnterStartup(p13) -- Line: 156
...
function u1.Client_EnterProjectileFlying(p16) -- Line: 177
    ...
    local v17 = p16.skillRunData.material["冰系尾迹"];

    if p16.skillRunData.runEvent["冰锥术Cast尾迹"] then
        p16.skillRunData.runEvent["冰锥术Cast尾迹"]:Disconnect();
        p16.skillRunData.runEvent["冰锥术Cast尾迹"] = nil;
    end;
    ...
    local v19 = p16.skillRunData.material["冰锥术法阵"];
    v19:ScaleTo(v18 * 0.5);
    ...
    local v22 = skillRunData.material["冰锥术_冰锥Emit和Enabled"];
    local v23 = skillRunData.material["冰锥术爆炸"];
    local v24, v25 = SkillCommon.scaleDualFromData(p16, SkillCommon.bandScaleOptsFromSkillData(p16));

    for _, descendant in pairs(v22:GetDescendants()) do
    ...
    FXUtil.Start_All_Emit(v22, 10);
    FXUtil.SetEmittersTrailsBeamsEnabled(v22, true);
    SkillCommon.playSoundLocal3D("音效-技能-冰系普攻_发射", v22:GetPivot().Position);
...
function u1.Client_ExitProjectileFlying(p29) -- Line: 280
...
function u1.Server_EnterProjectileFlying(u30) -- Line: 288
...
function u1.Server_ExitProjectileFlying(p39) -- Line: 339
...
function u1.Server_UpdateProjectileObstacleCheck(p41) -- Line: 351
...
function u1.Client_EnterExploding(p49, p50) -- Line: 390
    -- upvalues: FXUtil (copy), SkillCommon (copy), Debris (copy)
    ...
    local v52 = p49.skillRunData.material["冰锥术爆炸"];
    ...
    if v52 then
        ...
        SkillCommon.playSoundLocal3D("音效-技能-冰系普攻_爆炸", v51);
    end;
    ...
    if projectileModel then
        ...
        Debris:AddItem(projectileModel, 0.05);
        p49.skillRunData.Visual.projectileModel = nil;
    end;
end;

function u1.Server_EnterExploding(p53, p54) -- Line: 422
    ...
    if u57 then
        ...
        u57:start();
        FXUtil.BasePart_Size_Tween(hitbox, 0.1, v58, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, function() -- Line: 438
...
function u1.Server_EnterRecovery(p59) -- Line: 453
    p59:releaseControl();
end;

function u1.Client_EnterRecovery(p60) -- Line: 457
    local v61 = p60.skillRunData.material["冰系尾迹"];
    ...
    if p60.skillRunData.runEvent["冰锥术Cast尾迹"] then
        p60.skillRunData.runEvent["冰锥术Cast尾迹"]:Disconnect();
        p60.skillRunData.runEvent["冰锥术Cast尾迹"] = nil;
    end;
end;

function u1.onServerEvent(p62, p63) -- Line: 472
...
function u1.onProjectileHitServer(p65, p66, p67) -- Line: 493
    -- upvalues: SkillEventConst (copy), ProjectileImpact (copy)
    if not p66 then
        return;
    end;
...
u1.SoundList = { "音效-技能-冰系普攻_发射", "音效-技能-冰系普攻_爆炸" };
u1.AnimateList = { "技能释放动作3" };
u1.ResNameList = { "冰系尾迹", "冰锥术法阵", "冰锥术爆炸", "冰锥术_冰锥Emit和Enabled" };
u1.hitboxConfig = { {
        HitboxIndex = 1,
        PartName = "通用长方体",
