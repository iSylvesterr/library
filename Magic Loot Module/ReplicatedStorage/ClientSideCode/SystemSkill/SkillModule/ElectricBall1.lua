...
local function resolveLaunchPartFromCharacter(p2) -- Line: 122
    return p2:FindFirstChild("角", true);
end;

local function getPartPivotCF(p3) -- Line: 126
...
local function getLaunchCF(p4) -- Line: 139
...
local function getTargetPosition(p7) -- Line: 158
...
local function getSkillScale(p9) -- Line: 165
    -- upvalues: SkillCommon (copy)
    return SkillCommon.npcSummonBodySkillScale(p9);
end;

local function getExplosionHitboxDiameter(p10) -- Line: 169
    return p10 * 32;
end;

local function raycastGroundAlignedCF(p11, p12) -- Line: 177
    -- upvalues: FXUtil (copy)
    return FXUtil.GetGroundAlignedCF(p11, p12, "Ground", 3, 0.12);
end;

local function getGroundFxFlatHint(p13, p14) -- Line: 181
...
local function shouldPlayGroundExplosionFx(p18, p19, p20) -- Line: 197
    -- upvalues: FXUtil (copy)
...
local function hideModelBaseParts(p22) -- Line: 214
    -- upvalues: FXUtil (copy)
    if p22 then
...
local function setElectricBallParticleTrailEnabled(p23, p24) -- Line: 220
    if not p23 then
        return;
    end;
...
local function fadeProjectileVfx(p26) -- Line: 233
...
local function prepareProjectileVfx(p27) -- Line: 242
    -- upvalues: FXUtil (copy), setElectricBallParticleTrailEnabled (copy)
    if p27 then
...
local function emitParticleOnlyFxAt(p28, p29, p30) -- Line: 249
    -- upvalues: FXUtil (copy)
    if not p28 then
        return;
    end;
...
local function buildCubicBezierPoints(p31, p32) -- Line: 261
...
local function prepareFlightPlan(p36, p37) -- Line: 286
...
local function applyHitboxVisibility(p41, p42) -- Line: 295
    if not p41 then
        return;
    end;
...
local function isInProjectileFlying(p43) -- Line: 306
    local v44 = p43.GetCurrentState and p43:GetCurrentState() == "ProjectileFlying";

    return v44;
end;

local function stopClientProjectileFlight(p45) -- Line: 310
...
local function stopServerProjectileFlight(p47) -- Line: 322
...
local function playClientExplosion(p48, p49) -- Line: 330
    ...
    local v53 = skillRunData.material and skillRunData.material["电球爆炸"];
    ...
    if v60 and v59 then
        local v61 = skillRunData.material and skillRunData.material["电球爆炸地面特效"];
...
local function playServerExplosion(u62, p63) -- Line: 358
    ...
    if u65 and u65.hitbox then
        ...
        task.delay(0.2, function() -- Line: 366
            ...
            if u62.skillRunData and u65.hitbox then
                ...
                if v67 then
                    ...
                    u65:start();
                    task.delay(0.15, function() -- Line: 373
...
local function shouldTriggerExplosion(p70) -- Line: 389
    if not p70 then
        return false;
    end;
...
local function markExplosionTriggered(p71, p72, p73) -- Line: 397
...
local function tryTriggerExplosionClient(p74, p75, p76) -- Line: 403
...
local function tryTriggerExplosionServer(p79, p80, p81) -- Line: 416
...
function u1.Client_EnterStartup(p84) -- Line: 431
end;

function u1.Server_EnterStartup(p85) -- Line: 433
...
function u1.Client_EnterProjectileFlying(u87) -- Line: 441
    ...
    local v92 = material["电球"];
    local v93 = material["电球爆炸"];
...
function u1.Client_ExitProjectileFlying(p101) -- Line: 504
    -- upvalues: SkillEventConst (copy), playClientExplosion (copy)
...
function u1.Server_EnterProjectileFlying(u106) -- Line: 513
    ...
    task.delay(v113, function() -- Line: 534
...
function u1.Server_ExitProjectileFlying(p115) -- Line: 542
...
function u1.Server_EnterRecovery(p117) -- Line: 552
    p117:releaseControl();
end;

function u1.Client_EnterRecovery(p118) -- Line: 556
end;

function u1.onServerEvent(p119, p120) -- Line: 559
    -- upvalues: SkillEventConst (copy), playClientExplosion (copy)
...
function u1.onProjectileHitServer(p125, p126, p127) -- Line: 590
...
u1.AnimateList = { "电球" };
u1.ResNameList = { "电球", "电球爆炸", "电球爆炸地面特效" };
u1.hitboxConfig = { {
        HitboxIndex = 2,
        PartName = "通用球",
