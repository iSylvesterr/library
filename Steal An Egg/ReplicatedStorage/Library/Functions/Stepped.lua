...
local u11 = {
    Disconnect = function(p1) -- Line: 4, Name: Disconnect
    ...
    IsConnected = function(p2) -- Line: 18, Name: IsConnected
        return p2._connection ~= nil;
    end,

    Then = function(p3, p4) -- Line: 21, Name: Then
    ...
    Wait = function(p6) -- Line: 30, Name: Wait
        while p6:IsConnected() do
            task.wait();
        end;
    end,

    _Step = function(p7, p8) -- Line: 35, Name: _Step
        local _t = p7._t;
        local _duration = p7._duration;

        if not p7._timeUnrestricted then
...
return function(p12, p13, p14, p15) -- Line: 67
    ...
    u18._connection = RunService.Stepped:Connect(function(p19, p20) -- Line: 83
        -- upvalues: u18 (copy)
        u18:_Step(p20);
    end);
