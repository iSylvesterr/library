...
if v3 > 0 then
    ...
    local v22 = table.freeze({ "Common", "Rare", "Epic", "Legendary", "Mythic", "BrainrotGod", "Secret" });
    local v23 = {};
    ...
    for _, v in pairs(v2) do
        local v25 = v23[v.id] or (1 / 0);

        for _, v4 in ipairs(v.entries) do
            table.insert(v24, {
                name = v4.name,
                rarityId = v.id,
                rarityDisplay = v.displayName,
                rank = v25,
                dropWeight = v4.dropWeight
            });
        end;
    end;

    table.sort(v24, function(p26, p27) -- Line: 188
        if p26.dropWeight == p27.dropWeight then
            return p26.name < p27.name;
        end;

        return p26.dropWeight > p27.dropWeight;
    end);
    print("[AllAssetsByRarity] (least rare -> rarest)");

    for _, v in ipairs(v24) do
        print(string.format(" - %s  [%s]  weight=%.6f", v.name, v.rarityDisplay, v.dropWeight));
    end;
