...
local u2 = ModuleLoader(script._Index, {}, {
    typeCast = "CurrencyDir",
    rootName = "Currency.Dir",
    noPrint = true,
    shouldInject = true,
...
function v1.Types.CurrencyNameExists(u3) -- Line: 33
    -- upvalues: u2 (copy)
    ...
    return false, `Currency name "{u3}" does not exist in the Currency directory.`;
end;
...
if Constants.IS_STUDIO then
    for _, child in pairs(script._Index:GetChildren()) do
        ...
        assert(v4, v5);
        local v6, v7 = Interface.DefaultConfig(u2[child.Name]);
        local v8 = `Failed to validate config {child.Name}: {v7}`;
        assert(v6, v8);
    end;
end;

MakeTableStrict(u2, "Currency");
