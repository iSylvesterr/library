...
local function createPrompt(u8, p9, p10) -- Line: 83
    ...
    return function() -- Line: 448, Name: cleanup
        -- upvalues: u39 (ref), u40 (ref), u41 (ref), u42 (ref), u47 (copy), u13 (copy), u19 (ref)
        if u39 then
            u39:Disconnect();
        end;
...
local function onLoad() -- Line: 473
    -- upvalues: ProximityPromptService (copy), PlayerGui (copy), createPrompt (copy)
    ProximityPromptService.PromptShown:Connect(function(p48, p49) -- Line: 474
        -- upvalues: PlayerGui (ref), createPrompt (ref)
        if p48.Style == Enum.ProximityPromptStyle.Default then
            return;
        end;

        if p48:GetAttribute("HideUI") then
            return;
        end;
        ...
        local v50 = createPrompt(p48, p49, ProximityPrompts);
        p48.PromptHidden:Wait();
        v50();
    end);
end;

ProximityPromptService.PromptShown:Connect(function(p51, p52) -- Line: 474
    -- upvalues: PlayerGui (copy), createPrompt (copy)
    if p51.Style == Enum.ProximityPromptStyle.Default then
        return;
    end;

    if p51:GetAttribute("HideUI") then
        return;
    end;
    ...
    local v53 = createPrompt(p51, p52, ProximityPrompts);
    p51.PromptHidden:Wait();
    v53();
end);