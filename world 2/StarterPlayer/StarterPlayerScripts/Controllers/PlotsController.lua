...
local function getExpandSignTemplate(p38) -- Line: 151
    local SurfaceGui = p38:FindFirstChild("SurfaceGui");
...
local function setExpandSignModelVisible(p39, p40) -- Line: 166
    for _, descendant in p39:GetDescendants() do
        if descendant:IsA("BasePart") then
            descendant.Transparency = p40 and 0 or 1;
            descendant.CanCollide = p40;
        elseif descendant:IsA("Decal") then
            descendant.Transparency = p40 and 0 or 1;
        elseif descendant:IsA("SurfaceGui") or descendant:IsA("BillboardGui") then
            descendant.Enabled = p40;
        end;
    end;
end;
