...
return function(p5) -- Line: 19
    ...
    local u6 = {};
    local Night = require(ReplicatedStorage.SharedModules.TimeCycleData).Data.Night;

    if Night then
        for i in Night.Weathers do
            table.insert(u6, i);
        end;
    end;
    ...
    p5:RegisterType("moonType", {
        Validate = function(p11) -- Line: 32, Name: Validate
            -- upvalues: u10 (copy), u6 (copy)
            return u10(p11, true) ~= nil, string.format("%q is not a valid moon type. Valid types: %s", p11, table.concat(u6, ", "));
        end,
